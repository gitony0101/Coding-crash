...
- 在 C++ 中，float 和 double 都是浮点类型，用于表示小数。float 是单精度浮点类型，double 是双精度浮点类型。
  - double 的精度是 float 的两倍，即 double 可以表示更大范围的数值，并且可以保留更多的小数位。
  - float 和 double 的区别在于它们的存储空间和精度不同。float 占用 4 个字节（32 位），double 占用 8 个字节（64 位）。
