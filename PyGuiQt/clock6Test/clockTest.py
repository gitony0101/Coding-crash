...
class RootWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.blackmatt.setStyleSheet(
            "background-color: rgba(0, 0, 0, 30%);border-top-left-radius: 8px;border-top-right-radius: 8px;")
        self.blackmatt.hide()
    ...
    def loadWidgets(self) -> None:
        ...
        self.widgets[self.discover] = self.addTab(
            self.discover, _("Discover Packages"))
        self.updates = UpdateSoftwareSection()
        self.updates.setStyleSheet("QGroupBox{border-radius: 5px;}")
        globals.updates = self.updates
        self.widgets[self.updates] = self.addTab(
            self.updates, _("Software Updates"))
        ...
        self.widgets[self.uninstall] = self.addTab(
            self.uninstall, _("Installed Packages"))
        self.settingsSection = SettingsSection()
        self.widgets[self.settingsSection] = self.addTab(self.settingsSection, _(
            "WingetUI Settings"), addToMenu=True, actionIcon="settings")
        self.aboutSection = AboutSection()
        self.widgets[self.aboutSection] = self.addTab(self.aboutSection, _(
            "About WingetUI"), addToMenu=True, actionIcon="info")
        self.aboutSection = DebuggingSection()
        self.widgets[self.aboutSection] = self.addTab(
            self.aboutSection, _("WingetUI log"), addToMenu=True, actionIcon="buggy")
        ...
        self.adminButton.clicked.connect(
            lambda: (self.warnAboutAdmin(), self.adminButton.setChecked(True)))
        ...
        self.showHideButton.clicked.connect(lambda: (self.installationsWidget.setVisible(not self.installationsWidget.isVisible()), self.showHideButton.setIcon(
            QIcon(getMedia("collapse"))) if self.installationsWidget.isVisible() else self.showHideButton.setIcon(QIcon(getMedia("expand")))))
        ebw = QWidget()
        ...
        sct.activated.connect(lambda: (self.mainWidget.setCurrentIndex((self.mainWidget.currentIndex(
        ) + 1) if self.mainWidget.currentIndex() < 4 else 0), self.buttonBox.buttons()[self.mainWidget.currentIndex()].setChecked(True)))

        sct = QShortcut(QKeySequence("Ctrl+Shift+Tab"), self)
        sct.activated.connect(lambda: (self.mainWidget.setCurrentIndex((self.mainWidget.currentIndex(
        ) - 1) if self.mainWidget.currentIndex() > 0 else 3), self.buttonBox.buttons()[self.mainWidget.currentIndex()].setChecked(True)))
            ...
            if getSettings("DisablesystemTray"):
                if (tools.MessageBox.question(self, _("Warning"), _("There is an installation in progress. If you close WingetUI, the installation may fail and have unexpected results. Do you still want to quit WingetUI?"), tools.MessageBox.No | tools.MessageBox.Yes, tools.MessageBox.No) == tools.MessageBox.Yes):
                    if globals.updatesAvailable:
                        ...
                        globals.trayIcon.showMessage(_("Updating WingetUI"), _(
                            "WingetUI is being updated. When finished, WingetUI will restart itself"), QIcon(getMedia("notif_info")))
                        event.ignore()
                    else:
                        self.deleteChildren()
                        event.accept()
    ...
    def resizeEvent(self, event: QResizeEvent) -> None:
        ...
        try:
            s = self.infobox.size()
            self.infobox.move((self.width()-s.width())//2,
    ...
    def showEvent(self, event: QShowEvent) -> None:
        if (not self.isWinDark):
            ...
            if not self.appliedStyleSheet and globals.lightCSS != "":
                ...
                self.setStyleSheet(globals.lightCSS.replace(
                    "mainbg", "transparent" if r == 0x0 else "#ffffff"))
        else:
            r = win32mica.ApplyMica(self.winId(), win32mica.MICAMODE.DARK)
            if not self.appliedStyleSheet and globals.darkCSS != "":
                self.appliedStyleSheet = True
                self.setStyleSheet(globals.darkCSS.replace(
                    "mainbg", "transparent" if r == 0x0 else "#202020"))
