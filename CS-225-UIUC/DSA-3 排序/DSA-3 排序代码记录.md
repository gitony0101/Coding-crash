...
1. 部分节选，创建函数并开始循环，注意 `i=1`开始，留出了第0个元素，这样从一开始就能分成两个部分
      - 单独的那个元素，自成一列 ，可以看作是有序
2.   `if (arr[i] < arr[i - 1]) ` 如果当前数据元素比前一个小，符合我们的判断条件，可以往前寻找并插入了，进行插入排序
3. `for (j = i - 1; j >= 0 && temp < arr[j]; j--)`
     
      - `temp < arr[j]`这个条件很重要，确保插入的元素比前面的元素小，否则这层循环就能停下来
